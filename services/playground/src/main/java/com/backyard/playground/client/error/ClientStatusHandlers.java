...
/**
 * Shared status handler for RestClient — maps upstream HTTP error codes to typed exceptions that
 * {@code GlobalExceptionHandler} converts to clean API responses. Used as a method reference in each client config:
...
public final class ClientStatusHandlers {
    ...
    public static void handleError(HttpRequest request, ClientHttpResponse response)
        ...
        String message = String.format("[%d %s] during [%s] to [%s]: [%s]", status, response.getStatusText(),
                request.getMethod(), request.getURI(), new String(body, StandardCharsets.UTF_8));
        RestClientResponseException cause = new RestClientResponseException(message, response.getStatusCode(),
                response.getStatusText(), response.getHeaders(), body, StandardCharsets.UTF_8);
        throw switch (status) {
