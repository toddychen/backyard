 ...
 * Two modes:
 * <ul>
 * <li><b>Fixed params</b> — supply a {@code Map} of key/value pairs known at construction time (e.g.
 * {@code source=playground}).</li>
 * <li><b>API key</b> — supply a secret name, Spring {@code Environment}, local secrets file path, and GCP project ID.
 * The secret is loaded once at startup from a local JSON file when the {@code dev} profile is active, or from GCP
 * Secret Manager otherwise.</li>
 * </ul>
 */
public class QueryParamInterceptor implements ClientHttpRequestInterceptor {
    ...
    /**
     * Fixed params constructor — appends the given key/value pairs to every request.
     */
    ...
    /**
     * Creates an interceptor that appends both fixed query params and an API key loaded from a local secrets file (dev)
     * or GCP Secret Manager (stage/prod).
     *
     * @param params          fixed key/value query parameters appended to every request (e.g.
     *                        {@code source=playground})
     * @param apiKeyParamName query parameter name for the API key (e.g. {@code "key"})
    ...
    /**
     * Creates an interceptor that appends only an API key, with no additional fixed params.
     *
     * @param apiKeyParamName query parameter name for the API key (e.g. {@code "key"})
     * @param secretName      key name in the local JSON file and Secret Manager
     * @param environment     Spring environment used to detect active profiles
    ...
    public static QueryParamInterceptor onlyApiKey(
            ...
            String localFilePath,
            String gcpProjectId) {
        return withApiKey(Map.of(), apiKeyParamName, secretName, environment, localFilePath, gcpProjectId);
    }
    ...
    private static String loadApiKeyFromSecretManager(String secretName, String gcpProjectId) {
        try (SecretManagerServiceClient client = SecretManagerServiceClient.create()) {
            ...
            log.error("Could not load '{}' from GCP Secret Manager: {}", secretName, e.getMessage());
            return "";
        }
    }
}
