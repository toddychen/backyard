 ...
 * <h3>Resilience</h3>
 * <p>
 * Resilience4j circuit breaker and retry are applied transparently
 * by {@code ClientConfig.wrapWithResilience}. Each method gets its
 * own circuit breaker instance keyed as
 * {@code "yahoosports-<methodName>"}, so a failing endpoint does not
 * trip the breaker for other endpoints. Retry config is shared
 * under the {@code "yahoosports"} key (retry state is per-call, so
 * methods remain independent at runtime).
 *
 * <h3>Configuration</h3>
 * <p>
 * Circuit breaker and retry are configured in
 ...
 * <h3>404 handling</h3>
 * <p>
 * Yahoo returns 404 for unknown games. {@code ClientConfig}
 * translates these to {@code null} before resilience logic runs,
 * so 404s are not retried and do not count as circuit breaker
 * failures.
 */
@HttpExchange("/api/v8")
