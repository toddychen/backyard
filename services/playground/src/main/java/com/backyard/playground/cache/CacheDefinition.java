...
/**
 * Declares the configuration for a single named cache — type, TTL, and optional feature flags. All caches are
 * registered in {@link CacheDefinitionRegistry}.
 *
 * <p>
 * {@code dynamicTtl} signals that cached values implement {@link Expirable} and carry their own per-entry TTL. When
 * true, the cache infrastructure reads {@link Expirable#cacheDuration()} at insertion time to determine the TTL for
 * each entry individually.
 *
 * <p>
 * {@code maxSize} is only meaningful for caches with a LOCAL component (Caffeine). {@link CacheType#DISTRIBUTED} caches
 * ignore it — Redis relies on TTL-based eviction and memory limits instead.
 *
 * <p>
 ...
 *
 * <p>
 * Use the static factory methods to construct definitions without needing to supply irrelevant fields.
 */
public class CacheDefinition {
    ...
    private CacheDefinition(String name, CacheType type,
            Duration localTtl, Duration distributedTtl,
            boolean dynamicTtl, Integer maxSize) {
        this.name = name;
        this.type = type;
     ...
     *
     * @param localTtl       TTL for the local layer (Caffeine); should be
     *                       shorter than {@code distributedTtl} to bound
     *                       stale reads
     * @param distributedTtl TTL for the distributed layer (Redis)
     * @param maxSize        max entries in the local Caffeine cache
     */
    public static CacheDefinition multiLevel(String name, Duration localTtl, Duration distributedTtl, int maxSize) {
        return new CacheDefinition(name, CacheType.MULTI_LEVEL, localTtl, distributedTtl, false, maxSize);
    }

    /**
     * Dynamic per-entry TTL MULTI_LEVEL cache via {@link Expirable}. Both levels read {@link Expirable#cacheDuration()}
     * from the value at insertion time.
     */
