...
/**
 * {@link CacheManager} delegate for {@link CacheType#LOCAL} caches, backed by
 * Caffeine.
 *
 * <p>
 * Reads {@link CacheDefinition} from {@link CacheDefinitionRegistry} to
 * determine TTL and max size per cache. Supports both fixed TTL and dynamic
 * per-entry TTL (via {@link Expirable#cacheDuration()}) based on
 * {@link CacheDefinition#isDynamicTtl()}.
 *
 * <h3>Why TTL is baked in at construction, not per-put</h3>
 *
 * <p>
 * Caffeine has no per-put TTL API — TTL policy must be wired at construction
 * time via {@link Expiry}. There is no equivalent of Redis's
 * {@code SET key value EX ttl}. Dynamic TTL is therefore handled through
 * Caffeine's native {@code expireAfter(Expiry)} mechanism, reading
 * {@link Expirable#cacheDuration()} at insertion time.
 *
 * <p>
 * Not a Spring {@code @Component} — instantiated and registered as the
 * {@link CacheType#LOCAL} delegate inside {@link CacheConfig}.
 */
                ...
                Caffeine.newBuilder()
                        ...
                        .expireAfter(
                                new Expiry<Object, Object>() {
                                    ...
                                    // The warning log above will notify to fix the
                                    // misconfiguration.
                                    private long resolveTtl(Object key, Object value) {
                                        if (value instanceof Expirable e) {
                                            ...
                                            if (duration == null) {
                                                log.warn(
                                                        "cache '{}': key='{}' cacheDuration() returned null"
                                        ...
                                        log.warn(
                                                "cache '{}': key='{}' value of type '{}' does not implement"
