 ...
 * request via {@link ClientLocaleContextHolder} and Spring's built-in
 ...
 * <li>Run {@link Locale#lookup} against the set of {@link SupportedLocale}
 * candidates. Lookup uses BCP 47 prefix matching — {@code en-US} matches
 * {@code en}, and {@code zh-TW} matches {@code zh-TW} before falling back to
 * {@code zh-CN}.
 * <li>If no match is found (unsupported language) or the header is absent, fall
 * back to {@link SupportedLocale#EN}.
 * </ol>
 *
 ...
 *
 * <ul>
 * <li>{@code Vary: Accept-Language} — tells HTTP caches (CDNs, proxies) that
 * the response varies by the request locale, preventing a cached French
 * response from being served to an English requester.
 * <li>{@code Content-Language: <tag>} — tells the client which language the
 * response body is actually in.
 * </ul>
 *
 * <p>
 * Runs at {@code @Order(0)}, before {@link AccessLogFilter}
 * ({@code @Order(1)}), so the resolved {@code locale} MDC key is available in
 * every log line written during the request.
 */
@Component
@Order(0)
public class LocaleFilter implements Filter {
    ...
    // Pre-built candidate list — one Locale per SupportedLocale constant.
    // Used by Locale.lookup() on every request.
    private static final List<Locale> CANDIDATES = Arrays.stream(SupportedLocale.values())
            .map(SupportedLocale::toLocale).toList();
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        SupportedLocale resolved = resolve(header);

        // Extract region from the client's raw input, not the resolved supported
        // locale. resolved.toLocale() has no country (e.g. "en" from "en-US")
        // because our SupportedLocale candidates use bare language tags —
        // Locale.lookup() strips the region subtag during matching.
        ...
        // threads spawned by FanOut inherit this value at creation time from
        // the submitting request thread. See ClientLocaleContextHolder for
     ...
     * {@link SupportedLocale} using RFC 5646 BCP 47 lookup with fallback to
     * {@link SupportedLocale#EN}.
     */
    private static SupportedLocale resolve(String acceptLanguageHeader) {
        ...
        try {
            ...
            // Traditional and Simplified Chinese resolve to distinct entries.
            return SupportedLocale.fromLanguageTag(matched.toLanguageTag());
     ...
     * {@code Accept-Language} header that produced the resolved locale (e.g.
     * {@code "CA"} from {@code "zh,en-CA;q=0.9"} when {@code zh} is unsupported and
     * the match falls through to {@code en-CA}).
     *
     * <p>
     * We scan ranges in priority order and return the country subtag from the first
     * range whose language matches the resolved locale's language. This correctly
     * handles cases where the matching range is not the first one — e.g. if the
     * client prefers {@code zh} but we resolve to {@code EN} via {@code en-CA}, the
     * region should be {@code "CA"}, not {@code null}.
     *
     * <p>
     * Returns {@code null} when no matching range has a region subtag.
     */
    private static String extractRegion(String acceptLanguageHeader, SupportedLocale resolved) {
        if (acceptLanguageHeader == null || acceptLanguageHeader.isBlank())
            return null;
        try {
            List<Locale.LanguageRange> ranges = Locale.LanguageRange.parse(acceptLanguageHeader);
