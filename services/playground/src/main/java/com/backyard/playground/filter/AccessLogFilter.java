...
/**
 * Logs one access log line per request in Tomcat's Combined Log Format,
 * and sets MDC fields so logstash-logback-encoder emits them as top-level
 * JSON fields in the structured log output.
 *
 * <h3>Two outputs from one filter</h3>
 *
 * <p>
 * <b>message field</b> — Tomcat Combined Log Format, human-readable in
 * local dev:
 * <pre>
 ...
 *
 * <p>
 * logstash-logback-encoder serializes MDC entries as top-level fields in
 * the JSON log line. This allows Loki's {@code | json} pipeline to parse
 * them directly without regex extraction. A JSON string embedded inside
 * the {@code message} field would require an extra extraction step.
 */
@Component
@Order(1)
